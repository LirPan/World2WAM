...
class LiberoBatchAdapter(Dataset):
    """Wraps FastWAM RobotVideoDataset; adds future/current VAE latents from cache."""
    ...
    def __init__(
        ...
        *,
        ...
        dataset_name: str = "physics_world2wam_vb",
    ):
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        if self.cache is not None:
            if self.cache.has_current_latent(idx, self.anchor_action_idx):
                current_latent = self.cache.load_current_latent(idx, self.anchor_action_idx)
            if valid_future and self.cache.has_future_latent(
