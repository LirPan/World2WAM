...
run_libero_eval() {
    ...
    export ROOT_DIR
    MINIMAL_ROOT=${MINIMAL_ROOT:-"$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"}
    export MINIMAL_ROOT
    WORLD2WAM_LIBERO_TASK_SCRIPT=${WORLD2WAM_LIBERO_TASK_SCRIPT:-"${MINIMAL_ROOT}/scripts/libero/libero_single_task.sh"}
    export WORLD2WAM_LIBERO_TASK_SCRIPT
    USE_TMUX=${USE_TMUX:-1}
    export USE_TMUX
    ...
    if [ "$USE_TMUX" = "0" ]; then
        echo "USE_TMUX=0: running without tmux (direct background workers)"
    fi

    # Check for an existing tmux session
    if [ "$USE_TMUX" != "0" ] && tmux has-session -t $SESSION_NAME 2>/dev/null; then
    ...
    launch_task_on_pane() {
        ...
        tmux send-keys -t $SESSION_NAME:$pane_info "export MINIMAL_ROOT='${MINIMAL_ROOT}' STATUS_FILE='${status_file}' LOG_FILE='${log_file}' RESULT_FILE='${result_file}' EXP_NAME='${EXP_NAME}' && \
            bash '${WORLD2WAM_LIBERO_TASK_SCRIPT}' '${suite}' '${task_id}' '${gpu_id}'" C-m 2>/dev/null
        return 0
    }
    ...
    launch_task_direct() {
        ...
        local log_file="$TASK_LOG_DIR/${suite}_task${task_id}_gpu${gpu_id}.log"
        rm -f "$status_file"
        echo "[$(date '+%Y-%m-%d %H:%M:%S')] Direct launch: $suite task_id=$task_id on GPU$gpu_id"
        export STATUS_FILE="$status_file" LOG_FILE="$log_file" RESULT_FILE="$result_file"
        bash "${WORLD2WAM_LIBERO_TASK_SCRIPT}" "$suite" "$task_id" "$gpu_id" &
        echo $! > "${status_file}.pid"
    }
    ...
    launch_task() {
        ...
        if [ "$USE_TMUX" = "0" ]; then
            launch_task_direct "$suite" "$task_id" "$gpu_id"
        else
