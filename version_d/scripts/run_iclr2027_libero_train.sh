...
METHOD="$1"
SEED="$2"
case "$METHOD" in B1|B2|B3|B4|B5) ;; *) echo "invalid method: $METHOD" >&2; exit 2 ;; esac
...
RUN_DIR="$ROOT/runs/libero_train/$RUN_ID"
OUT="$ROOT/checkpoints/libero/$RUN_ID.pt"

mkdir -p "$ROOT/checkpoints/libero" "$RUN_DIR"
cd "$POLICY"
if [[ "$METHOD" == B1 ]]; then
  ...
  case "$METHOD" in
    ...
    B5) extra+=(--lambda-inv 0.05 --lambda-cycle 0.05 --enable-inverse true --enable-cycle true --gradient-mode project_conflicts) ;;
  esac
