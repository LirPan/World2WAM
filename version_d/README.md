...
## ICLR 2027 paper sprint

The current frozen experiment plan is in
`docs/ICLR2027_EXPERIMENT_SPRINT_PLAN.md`; the latest audited execution state
is in `docs/ICLR2027_SPRINT_STATUS_20260902.md`. The machine-readable protocol
is `protocols/iclr2027_paper_sprint.json`.

The full background pipeline is generated by
`scripts/build_iclr2027_manifest.py` and controlled by
`scripts/paper_sprint.py` through `plan`, `run`, `resume`, `status`, and
`summarize`. It covers 92 dependency-tracked jobs across RoboTwin, standard
LIBERO, and LIBERO-Plus, with GPU allow-listing, idle-process checks, atomic
states, artifact recovery, retries, disk protection, and 500-step training
checkpoints.
