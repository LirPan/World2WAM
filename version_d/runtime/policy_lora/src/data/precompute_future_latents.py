...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Deterministically sample this many global indices from each underlying dataset.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional JSON list or object with an 'indices' list; overrides prefix selection.",
    )
    ...
    selection_metadata: dict = {"mode": "prefix"}
    if args.indices_file:
        selection_payload = json.loads(Path(args.indices_file).read_text(encoding="utf-8"))
        raw_indices = (
            selection_payload.get("indices", [])
            if isinstance(selection_payload, dict)
            else selection_payload
        )
        selected_indices = sorted({int(index) for index in raw_indices})
        selection_metadata = {"mode": "indices_file", "source": str(Path(args.indices_file).resolve())}
    elif args.samples_per_dataset is not None:
        if args.samples_per_dataset <= 0:
            raise ValueError("--samples-per-dataset must be positive")
        multi_dataset = getattr(
            getattr(getattr(base_ds, "lerobot_dataset", None), "multi_dataset", None),
            "_datasets",
            None,
        )
        if not multi_dataset:
            raise RuntimeError(
                "Stratified selection requires base_ds.lerobot_dataset.multi_dataset._datasets"
            )
        selected_indices = []
        dataset_records = []
        offset = 0
        for dataset_idx, dataset in enumerate(multi_dataset):
            length = len(dataset)
            count = min(args.samples_per_dataset, length)
            rng = random.Random(f"{args.selection_seed}:{dataset_idx}")
            local_indices = sorted(rng.sample(range(length), count))
            selected_indices.extend(offset + index for index in local_indices)
            dataset_records.append(
                {
                    "dataset_index": dataset_idx,
                    "dataset_name": str(getattr(dataset, "repo_id", dataset_idx)),
                    "offset": offset,
                    "length": length,
                    "selected": count,
                }
        ...
        n_prefix = len(adapter) if args.max_samples is None else min(len(adapter), args.max_samples)
        selected_indices = list(range(n_prefix))

    if args.max_samples is not None:
        selected_indices = selected_indices[: args.max_samples]
    if not selected_indices:
        raise ValueError("No indices were selected for future-latent precomputation")
    if selected_indices[0] < 0 or selected_indices[-1] >= len(adapter):
        raise IndexError(
            f"Selected index range [{selected_indices[0]}, {selected_indices[-1]}] "
            f"is outside dataset length {len(adapter)}"
        )
    ...
    manifest_path = (
        ...
        / cache.dataset_name
        / (
            f"manifest_h{adapter.future_horizon}_n{n}"
            f"_shard{args.shard_id}of{args.num_shards}.json"
        )
    )
    skipped = 0
    saved = 0
    ...
    if args.shard_id == 0:
        ...
        payload["selection"] = selection_metadata
        selection_path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")

    shard_indices = selected_indices[args.shard_id :: args.num_shards]
    for idx in tqdm(shard_indices, desc=f"precompute shard {args.shard_id}/{args.num_shards}"):
        sample = adapter[idx]
