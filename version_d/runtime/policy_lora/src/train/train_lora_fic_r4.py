#!/usr/bin/env python3
"""R4 LoRA: behavior anchoring, hard-task curriculum, gradient control, and EMA.
...
class CurriculumHardSampler(Sampler[int]):
    """Sample hard/easy groups with a step-wise curriculum."""
    ...
    def __init__(
        ...
        *,
        ...
        if not 0.0 <= start_fraction <= end_fraction < 1.0:
            raise ValueError(
                "hard curriculum fractions must satisfy "
                f"0 <= start <= end < 1, got {start_fraction}, {end_fraction}"
            )
        self.hard_indices = torch.tensor(
            [idx for idx, is_hard in enumerate(labels) if is_hard], dtype=torch.long
        )
        self.easy_indices = torch.tensor(
            [idx for idx, is_hard in enumerate(labels) if not is_hard], dtype=torch.long
        )
        if not len(self.hard_indices) or not len(self.easy_indices):
            raise ValueError("hard curriculum requires both hard and easy samples")
    ...
    def hard_fraction(self, step: int) -> float:
        progress = min(max(float(step) / self.ramp_steps, 0.0), 1.0)
    ...
    def __iter__(self):
        for offset in range(self.num_samples):
            fraction = self.hard_fraction(self.start_step + offset)
            choose_hard = bool(
                torch.rand((), generator=self.generator).item() < fraction
            )
            pool = self.hard_indices if choose_hard else self.easy_indices
            position = int(
                torch.randint(len(pool), (), generator=self.generator).item()
            )
            yield int(pool[position].item())
...
class TrainableEMA:
    """Small EMA helper for the trainable LoRA parameters only."""
    ...
    def __init__(self, params: list[torch.nn.Parameter], decay: float) -> None:
        ...
        self.shadow = [p.detach().float().clone() for p in params]

    @torch.no_grad()
    def update(self) -> None:
        for shadow, param in zip(self.shadow, self.params):
            shadow.lerp_(param.detach().float(), 1.0 - self.decay)
    ...
    def average_parameters(self):
        ...
        try:
            ...
            with torch.no_grad():
                for param, backup in zip(self.params, backups):
                    param.copy_(backup)


def _batch_is_hard(batch: dict, keywords: list[str]) -> bool:
    prompts = batch.get("prompt") or batch.get("language") or []
    if isinstance(prompts, str):
        prompts = [prompts]
    text = " ".join(str(item) for item in prompts).lower()
    return any(keyword.lower() in text for keyword in keywords)


def _preserve_subset_metadata_prompt_getter(dataset):
    """Expose adapter metadata through Subset to avoid decoding videos at bootstrap."""
    if not isinstance(dataset, Subset):
        return dataset
    getter = getattr(dataset.dataset, "metadata_prompt_for_index", None)
    if not callable(getter):
        return dataset
    indices = dataset.indices

    def _metadata_prompt_for_index(subset_idx: int) -> str | None:
        return getter(int(indices[subset_idx]))

    dataset.metadata_prompt_for_index = _metadata_prompt_for_index
    return dataset


def _masked_token_mse(
    student: torch.Tensor,
    teacher: torch.Tensor,
    mask: torch.Tensor | None,
) -> torch.Tensor:
    per_token = F.mse_loss(
        student.float(), teacher.detach().float(), reduction="none"
    ).mean(dim=-1)
    if mask is None:
...
def _behavior_anchor_loss(
    wrapper: FastWAMWrapper,
    batch: dict,
    student_tokens: torch.Tensor,
) -> torch.Tensor:
    disable_adapter = getattr(wrapper.model.action_expert, "disable_adapter", None)
    if not callable(disable_adapter):
        raise RuntimeError("behavior anchoring requires PEFT disable_adapter()")
    with torch.no_grad(), disable_adapter():
        teacher_out = wrapper.forward_train(
            batch,
            use_future_latent_distill=True,
            policy_action_only_loss=True,
        )
    teacher_tokens = teacher_out["raw_outputs"].get("action_tokens")
    if teacher_tokens is None:
        raise RuntimeError("baseline teacher did not produce action tokens")
    return _masked_token_mse(
        student_tokens,
        teacher_tokens,
...
def _aligned_backward(
    *,
    ...
    conflict_world_scale: float = 1.0,
    max_world_to_action_ratio: float | None = None,
) -> dict[str, float | bool]:
    ...
    world_gradient_scale = float(conflict_world_scale if conflict else 1.0)
    if (
        max_world_to_action_ratio is not None
        and action_norm_sq.item() > 0.0
        and world_norm_sq.item() > 0.0
    ):
        norm_cap = float(max_world_to_action_ratio) * float(
            (action_norm_sq / world_norm_sq).sqrt().detach().item()
        )
        world_gradient_scale = min(world_gradient_scale, norm_cap)
    for param, action_grad, world_grad in zip(
        ...
        if world_grad is not None:
            world_grad = world_gradient_scale * world_grad
    ...
    return {
        ...
        "world_gradient_scale": world_gradient_scale,
    }


def train_lora_fic(cfg: dict, args: argparse.Namespace) -> None:
    cfg = normalize_config(cfg)
    if cfg.get("experiment_role") != "robotwin_lora_fic_r4_anchor":
        raise ValueError(
            "R4 entrypoint requires experiment_role=robotwin_lora_fic_r4_anchor; "
            "refusing to run against an R3/legacy configuration"
        )
    ...
    dataset = _preserve_subset_metadata_prompt_getter(
        Subset(dataset, usable_indices)
    )
    logger.info(
    ...
    curriculum_enabled = bool(cfg.get("hard_curriculum_enabled", False))
    curriculum_start = float(cfg.get("hard_curriculum_start_fraction", hard_fraction))
    curriculum_end = float(cfg.get("hard_curriculum_end_fraction", hard_fraction))
    curriculum_steps = int(cfg.get("hard_curriculum_ramp_steps", 1))
    ...
    if hard_keywords and 0.0 < hard_fraction < 1.0:
        weights, sampling_stats = build_balanced_weights(
            ...
            hard_fraction=curriculum_end if curriculum_enabled else hard_fraction,
    ...
    curriculum_labels: list[bool] | None = None
    if curriculum_enabled:
        if not hard_keywords:
            raise ValueError("hard curriculum requires hard_task_keywords")
        manifest = json.loads(sampling_manifest_path.read_text(encoding="utf-8"))
        curriculum_labels = [bool(value) for value in manifest["is_hard"]]
        if len(curriculum_labels) != len(dataset):
            raise RuntimeError("hard curriculum manifest length does not match dataset")
    ...
    lambda_anchor = float(cfg.get("lambda_behavior_anchor", 0.0))
    anchor_hard_scale = float(cfg.get("behavior_anchor_hard_scale", 1.0))
    ...
    conflict_world_scale = float(cfg.get("conflict_world_scale", 1.0))
    max_world_to_action_ratio_cfg = cfg.get("max_world_to_action_ratio")
    max_world_to_action_ratio = (
        None
        if max_world_to_action_ratio_cfg is None
        else float(max_world_to_action_ratio_cfg)
    )
    ...
    ema = TrainableEMA(backbone_params, ema_decay) if ema_enabled else None

    for epoch in range(start_epoch, int(cfg["num_epochs"])):
        generator = torch.Generator()
        generator.manual_seed(seed + epoch)
        if curriculum_labels is not None:
            sampler = CurriculumHardSampler(
                curriculum_labels,
                num_samples=len(dataset),
                start_fraction=curriculum_start,
                end_fraction=curriculum_end,
                ramp_steps=curriculum_steps,
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            student_tokens = fw_out["raw_outputs"].get("action_tokens")
            ...
            hard_sample = _batch_is_hard(batch, hard_keywords)
            anchor_weight = lambda_anchor * (
                anchor_hard_scale if hard_sample else 1.0
            )
            anchor_loss = action_loss.new_zeros(())
            if anchor_weight > 0.0:
                if student_tokens is None:
                    raise RuntimeError("behavior anchoring requires captured action tokens")
                anchor_loss = _behavior_anchor_loss(wrapper, batch, student_tokens)
            ...
            action_objective = (
                lambda_action * action_loss + anchor_weight * anchor_loss
            )
            ...
            if gradient_mode == "project_conflicts":
                gradient_metrics = _aligned_backward(
                    ...
                    conflict_world_scale=conflict_world_scale,
                    max_world_to_action_ratio=max_world_to_action_ratio,
                )
            else:
            ...
            global_step += 1
            current_hard_fraction = (
                sampler.hard_fraction(global_step)
                if isinstance(sampler, CurriculumHardSampler)
                else float(sampling_stats["hard_fraction"])
            )
            metrics = {
                ...
                "loss_anchor": float(anchor_loss.detach().item()),
                "lambda_anchor_effective": float(anchor_weight),
                ...
                "hard_sample_fraction": float(current_hard_fraction),
    ...
    def _save_final(path: Path, *, ema_applied: bool) -> None:
        save_world2wam_checkpoint(
            path,
            backbone_mode=backbone_mode,
            official_checkpoint=official_ckpt,
            ...
            meta={
                ...
                "ema_decay": ema_decay if ema_applied else None,
            },
            trainer_state=trainer_state,
        )

    if ema is None:
        _save_final(ckpt_dir / "world2wam_final.pt", ema_applied=False)
    else:
        _save_final(ckpt_dir / "world2wam_final_raw.pt", ema_applied=False)
        with ema.average_parameters():
            _save_final(ckpt_dir / "world2wam_final.pt", ema_applied=True)
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/robotwin_lora_fic_r4_anchor.yaml",
    )
